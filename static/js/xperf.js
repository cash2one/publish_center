...
function fn_list_endpoints()
{
    ...
    $(".loading").show();
    $.getJSON('/monitor/api/endpoints/', {q: qs, tags: tags, limit:limit, _r:Math.random()}, function(ret){
                $(".loading").hide();
                if (!ret.ok) {
                    alert(ret.msg);
                    return;
...
function fn_show_chart(counter)
{
    ...
    $.ajax({
        ...
        success: function(ret) {
            if (ret.ok) {
                setTimeout(function(){w.location='/monitor/chart/big?id='+ret.id;}, 0);
            } else {
                alert("请求出错了");
            }
        },
...
function fn_show_all(graph_type)
{
    ...
    $.ajax({
        ...
        success: function(ret) {
            if (ret.ok) {
                setTimeout(function(){w.location="/monitor/charts?id="+ret.id+"&graph_type="+graph_type;}, 0);
            }else {
                alert("请求出错了");
            }
        },
